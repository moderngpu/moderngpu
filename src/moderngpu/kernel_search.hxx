...
template<typename bounds_t bounds, typename launch_arg_t = empty_t,
...
void sorted_search(needles_it needles, int num_needles, haystack_it haystack,
  ...
  auto k = [=]MGPU_DEVICE(int tid, int cta) {
    ...
    merge_pair_t<type_t, vt> merge = cta_merge_from_mem<bounds, nt, vt>(a_keys,
      b_keys, range, tid, mgpu::less_t<type_t>(), shared.keys);

    // Store the needle indices to shared memory.
    iterate<vt>([&](int i) {
      if(merge.indices[i] < merge.a_count()) {
  ...
  cta_transform(k, num_needles + num_haystack, context);
}
